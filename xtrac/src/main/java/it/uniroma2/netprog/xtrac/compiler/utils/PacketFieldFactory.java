...
public class PacketFieldFactory {
    ...
    private PacketFieldFactory() {
        ...
        packetFields.put("packet.metadata", new PacketField("packet.metadata", "0", "4"));
    }
    ...
    public PacketField getPacketFieldFromIndexAccess(String fieldString, String indexToken) throws InvalidValueException, UnknownOpcodeException {
        ...
        if(elements.length == 1) {
            return new PacketField(packetFieldType.toString(), elements[0], "1");
        }
        else {
            ...
            return new PacketField(packetFieldType.toString(), elements[0], len);
        }
    }
}